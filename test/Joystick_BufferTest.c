 ...
 /*
	* This modified version of Joystick.c is under GPL-3.0 or later
		Major Changes:
		 ...
		 - (2026 May) Implemented circular queue for RX
...
static void CircularQueue_push2bytes(CircularQueue_t *my_queue, uint16_t value){
	CircularQueue_push(my_queue, (value >> 8) & 0xFF);
	CircularQueue_push(my_queue,  value 	  & 0xFF);
}

static void CircularQueue_push4bytes(CircularQueue_t *my_queue, uint32_t value){
	CircularQueue_push(my_queue, (value >> 24) & 0xFF);
	CircularQueue_push(my_queue, (value >> 16) & 0xFF);
	CircularQueue_push(my_queue, (value >> 8)  & 0xFF);
	CircularQueue_push(my_queue,  value        & 0xFF);
}

static void ISR_RX_Mock(CircularQueue_t *my_queue){	
	// HOME
	CircularQueue_push(my_queue, 0);
	CircularQueue_push2bytes(my_queue, 0x1000);
	CircularQueue_push4bytes(my_queue, 140);

	CircularQueue_push(my_queue, 0);
	CircularQueue_push2bytes(my_queue, 0x00);
	CircularQueue_push4bytes(my_queue, 200);

	// HAT LEFT
	CircularQueue_push(my_queue, 1);
	CircularQueue_push2bytes(my_queue, 0x06);
	CircularQueue_push4bytes(my_queue, 300);

	CircularQueue_push(my_queue, 1);
	CircularQueue_push2bytes(my_queue, 0x08);
	CircularQueue_push4bytes(my_queue, 360);

	// HAT RIGHT
	CircularQueue_push(my_queue, 1);
	CircularQueue_push2bytes(my_queue, 0x02);
	CircularQueue_push4bytes(my_queue, 500);

	CircularQueue_push(my_queue, 1);
	CircularQueue_push2bytes(my_queue, 0x08);
	CircularQueue_push4bytes(my_queue, 560);

}

static void TimedCommandList_Init(TimedCommands *cmds, CircularQueue_t *my_queue){
	uint8_t p0, p1, p2, p3;
	uint8_t i = 0;
	while(CircularQueue_pop(my_queue, &p0)){
		// target
		cmds[i].target  = p0;

		// value
		CircularQueue_pop(my_queue, &p1);
		CircularQueue_pop(my_queue, &p0);
		cmds[i].value   = ((uint16_t)p1 << 8)  | (uint16_t)p0;
		
		// time
		CircularQueue_pop(my_queue, &p3);
		CircularQueue_pop(my_queue, &p2);
		CircularQueue_pop(my_queue, &p1);
		CircularQueue_pop(my_queue, &p0);
		cmds[i++].time_ms = ((uint32_t)p3 << 24) | ((uint32_t)p2 << 16) | ((uint32_t)p1 << 8) | (uint32_t)p0;
	}
}
...
int main(void) {
	...
	CircularQueue_init(&my_queue, buffer_data, BUFFER_SIZE);
	ISR_RX_Mock(&my_queue);
	TimedCommandList_Init(mycmds, &my_queue);
	SetupTimer1();
